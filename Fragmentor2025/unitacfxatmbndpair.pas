...
uses
  Classes, SysUtils, contnrs, UnitFragmentBase, UnitACFXBase, UnitAtomSymbol, UnitMoleculeBase,
  UnitMoleculeFrg, unitAtomAndBondType, U_TYPE_GRAPHES, UnitAtomBase; //UnitBondBase,

type

  { TACFXAtmBndPair }

  TACFXAtmBndPair = class(TACFXBase)
  private
    //RepBC: TBondBase;
  protected
...
procedure TACFXAtmBndPair.PathToString(Mol: TMoleculeFrg; TOL: TObjectList);
var
   u, v: Node;
...
begin
     ...
     for u:=1 to fNP do begin
         //
         stail:=fPath[u,1]; shead:=fPath[u,2];
         ttail:=fPath[u,fL[u]]; thead:=fPath[u,fL[u]-1];
         //
         PB:=Mol.FindBond(stail,shead);
         PA:=Mol.AtmSet[stail];
         RepAC.StereoParity:=Mol.AtStereoParity(PA);
         //RepBC.StereoBond:=Mol.BdStereo(PB);
         FORWD:=RepAC.AtomString[stail]+PB^.S+IntToStr(fV[u]);
         PB:=Mol.FindBond(thead,ttail);
         PA:=Mol.AtmSet[ttail];
         RepAC.StereoParity:=Mol.AtStereoParity(PA);
         //RepBC.StereoBond:=Mol.BdStereo(PB);
         FORWD:=FORWD+PB^.S+RepAC.AtomString[ttail];
         //
         LocST.Add('('+FORWD+')');
     end;
...
procedure TACFXAtmBndPair.PathToString(Mol: TMoleculeFrg; LOPath: TIACHolder;
...
constructor TACFXAtmBndPair.Create;
begin
  inherited Create;
  //RepBC := TBondBase.Create;
end;

constructor TACFXAtmBndPair.Create(s: TStringList);
begin
  inherited Create;
  //RepBC   := TBondBase.Create;
  fFrgLst := s;
...
destructor TACFXAtmBndPair.Destroy;
begin
  //FreeAndNil(RepBC);
  inherited Destroy;
end;
