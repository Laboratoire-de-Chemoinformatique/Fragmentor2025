...
uses
  Classes, SysUtils, strutils, contnrs, UnitFragmentBase, UnitACFXBase, UnitMoleculeFrg,
  UnitMoleculeBase, unitAtomAndBondType, U_TYPE_GRAPHES, unitatombase;
...
procedure TACFXPathAtom.PathToString(Mol: TMoleculeFrg; TOL: TObjectList);
var
   u, v, vmax: Node;
   FORWD: string;
begin
     for u:=1 to fNP do begin
         FORWD:='';
         vmax:=fL[u];
         for v:=1 to vmax do begin
             RepAC.StereoParity:=Mol.AtStereoParity(fPath[u,v]);
             FORWD:=FORWD+RepAC.AtomString[fPath[u,v]];
         end;
         //
         (TOL.Items[fV[fPath[u,vmax]]-LenMin+1] as TStringList).Add('('+FORWD+')');
     end;
end;

procedure TACFXPathAtom.PathToString(Mol: TMoleculeFrg; LOPath: TIACHolder;
...
constructor TACFXPathAtom.Create;
begin
     inherited Create;
end;

constructor TACFXPathAtom.Create(s: TStringList);
begin
     inherited Create;
     fFrgLst:=s;
end;

destructor TACFXPathAtom.Destroy;
begin
     Inherited Destroy;
end;
